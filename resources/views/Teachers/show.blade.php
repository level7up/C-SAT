...
<hr>
    ...
    <script type="text/javascript">
        ...
        var lastIndex = fd ;
        firebase.database().ref('com/ta').child(lastIndex).once('value').then(function(snapshot) {
        ...
        firebase.database().ref().child('/Reviews/'+ lastIndex).once('value').then(function(snapshot) {
            ...
            $.each(value, function(index, value){
                if(value) {
                    htmls.push('<tr>\
                        <td><img style="width:50px" class="rounded-circle" src="'+value.imageurl+'"><a class="btn-link btn-primary">'+ value.name +'</a></td>\
